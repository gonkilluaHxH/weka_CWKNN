 ...
 */
public class IBK_WKS extends AbstractClassifier implements OptionHandler, UpdateableClassifier, WeightedInstancesHandler,
	...
	public static final Tag[] TAGS_WEIGHTING = { new Tag(WEIGHT_NONE, "No distance weighting"),
			...
			new Tag(WEIGHT_WKS, "Weight by WKS") };
	...
	/**
	 * IBK_WKS classifier. Simple instance-based learner that uses the class of the
	 ...
	 * WEIGHT_INVERSE, WEIGHT_SIMILARITY, or WEIGHT_WKS
	 *
	...
	public static  Instance WKSWeight(Instance n) throws Exception { // m3afne
		...
		n.setWeight(final_result);
        return n;
	}

	/**
	 * Sets the distance weighting method used. Values other than WEIGHT_NONE,
	 * WEIGHT_INVERSE,or WKS ,or WEIGHT_SIMILARITY will be ignored.
	 *
	 ...
	 */
	public Instances getInstancesTraining() {
		return m_Train;
	}

	public void setInstancesTraining(Instances train) {
		m_Train = train;
	}
	...
	public String toString() {
		...
		if (m_Train == null) {
			return "IBK_WKS: No model built yet.";
		}
	...
	protected double[] makeDistribution(Instances neighbours, double[] distances) throws Exception {
		...
		for (int i = 0; i < neighbours.numInstances(); i++) {
			...
			case WEIGHT_SIMILARITY:
				...
				break;
			case WEIGHT_WKS:
				current = WKSWeight(neighbours.instance(i));
				weight = current.weight();// ta3a
	        	weight = distances[i]/(1/weight);
...
//didnt understand the algo.
	...
	public static void main(String[] argv) {
		runClassifier(new IBK_WKS(), argv);
	}
}
